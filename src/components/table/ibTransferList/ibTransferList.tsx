...
import { useIbWalletTransactions } from "@/hooks/useIbWallet";


interface Props { }

const IBTransferList = (props: Props) => {
  // Showcasing the use of search params cache in nested RSCs
  const router = useRouter();
  const { data, isLoading, params, updateParams, total } = useIbWalletTransactions();
    ...
    return (
      <DataTable component={
        <div className="grid grid-cols-1 sm:grid-cols-2 md:flex items-center gap-2">
          <Select value={params.transaction_type} onValueChange={(value) => updateParams("transaction_type", value)}>
            <SelectTrigger 
              className="md:w-fit col-span-2 w-full py-0  bg-muted-foreground/5 rounded-md text-sm">
              <SelectValue placeholder="type" />
            </SelectTrigger>
            <SelectContent>
              {["all", "transfer_out", "commission",].map((type) => (
                <SelectItem key={type} value={type}>
                  
                  {type == "all" ? "All Types" : type=="transfer_out"?"wallet transfer":type}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
